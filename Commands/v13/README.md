...
|3||`cy.intercept('/api/promotions').as('promotions);`<br/><br/>`cy.visit('/')`<br/>`.get('button')`<br/>`.click();`<br/><br/>`cy.wait('@promotions');`<br/><br/>`cy.get('input[title="promo-code"]').should('be.visible');`|
